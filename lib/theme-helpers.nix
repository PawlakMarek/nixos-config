...
{lib, ...}: rec {
  ...
  generateThemeName = variant: accent: let
    capitalizedAccent = lib.toUpper (builtins.substring 0 1 accent) + 
  ...
  generateCursorTheme = variant: accent: let
    variantCap = lib.toUpper (builtins.substring 0 1 variant) + 
                 builtins.substring 1 (builtins.stringLength variant) variant;
    accentCap = lib.toUpper (builtins.substring 0 1 accent) + 
                builtins.substring 1 (builtins.stringLength accent) accent;
  in "catppuccin-${variant}-${accent}-cursors";
  ...
  isValidAccent = accent:
    builtins.elem accent ["rosewater" "flamingo" "pink" "mauve" "red" "maroon" 
  ...
  variantToPackageAttr = variant: accent: 
    "${variant}${lib.toUpper (builtins.substring 0 1 accent)}${builtins.substring 1 (builtins.stringLength accent) accent}";
}